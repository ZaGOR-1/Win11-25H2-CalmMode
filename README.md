...
- кнопка **Run Audit (safe)** — read-only прогін: нічого не змінює, лише показує звіт;
- кнопка **Apply…** — запитує підтвердження і запускає застосування **від адміністратора** (UAC); перед змінами рушій робить backup і restore point;
- кнопка **Close** — вийти.

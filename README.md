...
| `-OpenReport` | off | Після запису звітів відкрити HTML-звіт у браузері за замовчуванням. Opt-in; ігнорується разом із `-NoReport` (немає що відкривати) |
...
- кнопка **Apply…** — спершу робить швидкий внутрішній Audit і показує **скільки приблизно пунктів зміниться**, потім запитує підтвердження і запускає застосування **від адміністратора** (UAC); перед змінами рушій робить backup і restore point; результат так само показується у вікні, а звіт і `rollback.reg` зберігаються на Робочому столі;
...
- кнопка **Undo last Apply** — знаходить останню теку `…-Apply-…` на Робочому столі й імпортує її `rollback.reg` (з підтвердженням і UAC); повертає **лише реєстр**, не Appx. Якщо теки на Робочому столі немає (кастомний `-ReportPath`, перенаправлений Робочий стіл) — пропонує **вибрати теку** з `rollback.reg` вручну;
- кнопка **Close** — вийти.
...
Кожен звіт додатково містить:

- **`Run configuration` (Preflight)** — фіксує, які блоки були увімкнені, які окремі твіки вимкнені
  та звідки походив вибір (`-ConfigPath` / `-Skip` / `-Only` / типові налаштування). Робить прогін
  відтворюваним: видно, що саме застосовувалось.
- **Банер «Pending reboot» у HTML** — якщо в системі вже є відкладене перезавантаження, угорі звіту
  зʼявляється помітне попередження (частина політик повністю діє лише після reboot).
...
> Примітка: якщо політика не підходить для поточної редакції/збірки (її Apply пропустив би),
> то в `Audit`/`Verify` вона теж позначається `Skipped`, а не `WouldChange`/`VerifyFail` —
> щоб read-only режими були узгоджені з тим, що реально робить `Apply`.

## Коди виходу (exit codes)

Скрипт повертає код виходу, щоб його можна було використовувати в планувальнику завдань або CI:

| Код | Значення |
|---:|---|
| `0` | Чисто — без `Error`/`VerifyFail` |
| `1` | Помилка параметрів або префлайту (наприклад, конфлікт `-Skip`/`-Only`, невідомий ключ блоку, не знайдено `-ConfigPath`/`-RestoreFrom`, `Apply` без адміна) |
| `2` | Завершено, але є щонайменше один `Error` або `VerifyFail` — дивіться звіт |

> Зауваження про `Apply -ThenVerify`: авто-`Verify` одразу після `Apply` підтверджує, що значення
> **записані в реєстр**. Він **не** доводить, що Windows вже застосувала policy у UI — частина політик
> повністю діє лише після перезавантаження. Тому для остаточної перевірки запускайте окремий `Verify`
> після reboot. Код `2` від pre-reboot прогону не обовʼязково означає невдалий запис.

...
Проєкт використовує просту схему версій: `vMAJOR.MINOR` з patch-релізами `vMAJOR.MINOR.PATCH`
для невеликих виправлень.
...
- Launcher `Win11-25H2-CalmMode-GUI.cmd` більше не створює тимчасовий `.vbs`, не викликає
  `cscript.exe` і не запускає cleanup через `del`; GUI стартує напряму через `start /b`, що
  прибирає друге миготіння console-вікна і не має небезпечного wildcard-cleanup шляху.
- Launcher віддає пріоритет `Sysnative` PowerShell із 32-bit контексту, перевіряє наявність
  Windows PowerShell 5.1 і збережений як UTF-8 без BOM з CRLF, щоб `.cmd` коректно стартував
  з `@echo off`.
- Pester-тести тепер одразу повідомляють, що потрібен Pester 4.0 або новіший, замість десятків
  шумних помилок під локальним Pester 3.x.
- Тестовий helper більше не використовує `Invoke-Expression`: локальні function definitions
  з основного скрипта завантажуються через `[scriptblock]::Create(...)` і dot-source.

## [v2.12] - 2026-06-24

### GUI

- Почато реалізацію плану покращення GUI: додано компактну верхню статус-панель з build/edition/mode
  та badges безпеки, двопанельну навігацію `Категорії -> Твіки`, а також колонки `Надійність`,
  `Ризик` і `Реєстр` для кожного твіка.
- Видимий GUI тепер показує класифікацію `Official` / `UISetting` / `RequiresVerification` /
  `BestEffort` / `Deprecated` як кольорові рядки й risk-labels, але внутрішня модель config
  лишається сумісною з попереднім форматом `blocks` + `disabledTweaks`.
- Панель деталей вибраного твіка тепер показує registry path/name/value, support/build/edition,
  risk, admin requirement, restart note і rollback note; Appx cleanup окремо попереджає, що
  `rollback.reg` не повертає видалені Appx пакети.
- `Apply` вимкнений до успішного `Audit` для поточного вибору; будь-яка зміна категорій/твікiв
  скидає gate і вимагає повторного Audit.
- Вікно результатів отримало вкладки `Summary`, `Details` і `Raw log`: summary рахує ключові
  статуси, details лишає таблицю з фільтром attention, raw показує фактичний JSON-звіт.
- Покращено читабельність списків GUI: ширини колонок тепер адаптуються до resize/splitter,
  ліва панель стала ширшою за замовчуванням, а довгі назви категорій і твікiв мають tooltip-и.
- Дефолтний розмір GUI збільшено, а splitter стартує ближче до середини вікна, щоб таблиця
  `Tweaks` одразу показувала колонки `Confidence`, `Risk` і `Registry`.

### Документація

- Кореневі `README.md` і `README_EN.md` скорочено до стартових сторінок.
- Детальні інструкції винесено в окремі файли `docs/USAGE_*`, `docs/REPORTS_ROLLBACK_*`,
  `docs/POLICIES_*`.
- Release archive тепер включає `docs/`, а `checksums.txt` рахує файли рекурсивно.

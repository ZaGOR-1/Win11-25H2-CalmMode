...
> Примітка: під час цієї сесії закрито **всі High (розділ 5), Medium (розділ 6) і Low (розділ 7)
> пункти**, а також початковий блокер (застарілий zip/checksums). Стан описує репозиторій уже
> **після** цих фіксів. Лишилися тільки ручні кроки публікації релізу (`git push --tags` + GitHub Release).
...
| CI (GitHub Actions) | **8.5 / 10** (Pester запінено, реліз уніфіковано; лишається разова перевірка PSSA у CI) |
| Release hygiene | **10 / 10** |
| Готовність до публічного релізу | **9.5 / 10** (лишилися тільки ручні кроки публікації) |
...
Усі пункти розділів 5 (High), 6 (Medium) і 7 (Low) закрито. Перед публікацією лишилися лише
ручні кроки maintainer-а: `git push --tags` і створення GitHub Release. Одна разова перевірка
(PSScriptAnalyzer без Warning) виконається автоматично в CI на чистому раннері.
...
## 7. Low priority / polish — ✅ ОПРАЦЬОВАНО

- **Uppercase проти lowercase у форматі хешів — ✅ знято.** Разом із 5.1: локально й у CI тепер єдиний
  скрипт `New-ReleaseArchive.ps1` → скрізь однаковий uppercase-формат.
- **`checksums.txt` писався з CRLF — ✅ виправлено.** Build-скрипт тепер пише `checksums.txt` і `.zip.sha256`
  з LF та UTF-8 без BOM (`[System.IO.File]::WriteAllText` з `UTF8Encoding($false)`). Перевірено: `0x0d = 0`,
  без BOM; `sha256sum -c checksums.txt` → усі OK.
- **README ~38 KB — ✅ свідомо лишено в одному файлі.** За рішенням maintainer-а детальні таблиці політик
  лишаються в README: для security-tool прозорість «що саме чіпає скрипт» в одному файлі важливіша за стислість.

---

## 8. README / documentation fixes — ✅ УСІ ВИПРАВЛЕНО

- **`CLAUDE.md` — ✅ виправлено.** Старе ім'я `Win11-25H2-CalmMode-v2.1.ps1` у блоці тестування (2 згадки)
  замінено на стабільне `Win11-25H2-CalmMode.ps1`.
- **`RELEASE.md` — ✅ виправлено.** Перенумеровано (1–7), додано крок верифікації архіву, узгоджено з версійним ім'ям zip.
- **README — ✅ звірено.** Відповідає коду (параметри, дефолти, статуси, Appx limitations, rollback, Target Release
  pinning, Manual Update mode, Gaming-секція). Жорстко прописаного імені архіву не містить, тож уніфікація релізу нічого не ламає.
- **`CHANGELOG_UA.md` / `CHANGELOG_EN.md` — ✅ доповнено.** Оскільки тег `v2.2` тепер включає release/CI-коміти цієї сесії,
  під v2.2 додано стислий запис про зміцнення release-інструментарію (SHA256 через .NET, LF/без BOM, без `.gitignore`,
  CI викликає той самий скрипт, пін Pester 4.10.1).
...
- [x] Уніфікувати формат хешів (uppercase скрізь)
- [x] checksums.txt / .sha256 з LF та без BOM
- [x] README свідомо лишено єдиним файлом (transparency)
- [ ] `git push --tags` і створити GitHub Release (вручну, рішення maintainer-а)
- [ ] Перевірити PSScriptAnalyzer (Error,Warning) у CI на чистому раннері (авто)
- [ ] (опц., docs) Оновити старі згадки імені файлу `…-v2.1.ps1` у CLAUDE.md
```

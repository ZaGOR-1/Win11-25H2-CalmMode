...
> Статус (2026-06-20): `VERSION = 2.5` (в розробці; останній реліз — тег `v2.4`). Зроблено 5A.
> Гейти зелені — parse (рушій + GUI), PSScriptAnalyzer CLEAN, Pester **38/38**, Audit `EXIT=0`,
> forbidden patterns відсутні. Лишилось **5B + 5C** (+ закрити реліз v2.5).
...
2. ~~**5A (5.1 → 5.2 → 5.3)**~~ ✅ виконано (v2.5, ще не зарелізено).
3. **Реліз v2.5** — закомітити, оновити AUDIT.md, ZIP, тег `v2.5`.
4. **5B (5.4, 5.5)** — нові параметри запуску; обовʼязково тести. ← наступне з фіч
5. **5C (5.6 → 5.7 → 5.8)** — обережно, з документацією, opt-in і підтвердженнями.

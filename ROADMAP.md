...
> Статус (2026-06-19): `VERSION = 2.4` (в розробці; останній реліз — тег `v2.3`). Гейти зелені —
> parse (рушій + GUI), PSScriptAnalyzer CLEAN, Pester 36/36, Audit `EXIT=0`, forbidden patterns
> відсутні. Наступне — Фаза 5.
...
## ✅ Фаза 4 — якість і покриття (виконано 2026-06-19, v2.4)

- [x] **4.1 Тести на конфіг-механізм** — Pester (інтеграційні, через дочірній `powershell.exe`): структура `-ExportCatalog`; фільтр `-ConfigPath` (вимкнений блок → 0 результатів, вимкнений твік → `Skipped`); Audit видає лише відомі статуси; exit 1 на неіснуючому конфігу. GUI `-SelfTest` розширено перевіркою формування конфіга + round-trip. **36 тестів** (було 26).
- [x] **4.2 GUI: показ результату у вікні** — Audit/Apply читають згенерований JSON і показують його в `DataGridView` («Needs attention» + перемикач *Show all* + зведення по статусах). Audit більше не відкриває консоль; Apply зберігає звіт+`rollback.reg` на Робочому столі.
- [x] **4.3 GUI: прибирання temp-конфігів + дрібниці** — temp-конфіги видаляються після `-Wait`-прогону + чистка залишків на старті; кнопки **Save/Load config**; базова HiDPI (`SetProcessDPIAware` + `AutoScaleMode=Dpi`).
- [x] **4.4 Статус-рядки** — додано `Get-KnownStatuses` (єдине джерело істини) + guard у `Add-Result` (verbose на невідомий статус) + тест. Масову заміну 40+ call-site **свідомо не робили**: ті самі літерали означають різне в полях Status/Confidence/Support, тож сліпа заміна небезпечна — guard+тест дають захист від друкарських помилок без ризику.
...
2. ~~**Фаза 4** — тести на конфіг + прозорість результату в GUI~~ ✅ виконано (v2.4).
3. **Реліз v2.4** — закомітити, оновити AUDIT.md, зібрати ZIP, тег `v2.4`. ← наступне
4. **5.5 / 5.6** — підсумок наприкінці прогону + кращий HTML-звіт.
5. **5.1, 5.3, 5.2** — зручність CLI/потоку.
6. **5.4, 5.7, 5.8** — обережно, з документацією й opt-in.

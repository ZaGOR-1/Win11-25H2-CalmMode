...
> Статус (2026-06-20): у дереві `VERSION = 2.9`, гейти зелені — parse OK, PSScriptAnalyzer CLEAN,
> Pester **68/68**, Audit `EXIT=0` (0 `TerminatingError`), forbidden patterns відсутні.
> Останній **зарелізений** тег — `v2.6`. Фази P1–P4 (`v2.6.1`→`v2.9`) реалізовані, але **ще не
> закомічені й не отеговані**. Деталі зробленого — в `AUDIT.md` і `CHANGELOG_UA/EN`.

---

## 0. Найперше — процес (не код)

- [ ] **Зарелізити вже зроблене.** У дереві 4 готові версії без тегів: `v2.6.1` (exit-код),
  `v2.7` (прозорість/дедуп), `v2.8` (GUI UX), `v2.9` (типи реєстру + кеш). Закомітити логічними
  групами (feat+tests, потім docs), зібрати ZIP із SHA256, отеговати, запушити.
  **Чи треба:** ✅ так — це єдина «справжня» незавершеність; робота вже існує, лишається випустити.
  🟢
...
1. **Зарелізити `v2.6.1`→`v2.9`** (розділ 0) — випустити вже зроблене. ← реально наступне
2. **VM-валідація Apply** (розділ 1) — коли буде VM; закриває головну прогалину довіри.

...
> Статус (2026-06-19): `VERSION = 2.4` (в розробці). Останній **запушений тег — `v2.3`**.
> Робота v2.4 закомічена локально, але ще **не запушена і не позначена тегом**.
> Гейти зелені — parse (рушій + GUI), PSScriptAnalyzer CLEAN, Pester **36/36**, Audit `EXIT=0`,
> forbidden patterns відсутні. Лишилась **Фаза 5** (+ закрити реліз v2.4).
...
## ⏳ Реліз v2.4 (хвіст — зробити перед Фазою 5) 🟡

Робота v2.4 уже в локальному коміті, але реліз не закрито:

- [ ] **Запушити `main`** на origin (`ahead 1`). 🟢
- [ ] **Оновити `AUDIT.md`** — зараз описує v2.3; має відображати v2.4 (GUI-результат у вікні, Save/Load, тести 36, HiDPI). 🟢
- [ ] **Зібрати й перевірити ZIP** через `New-ReleaseArchive.ps1` (без `.git`/`.vs`/`settings.local`/звітів; SHA256). 🟢
- [ ] **Тег `v2.4`** після пушу (release-воркфлоу збирається по тегу; наявні `v2.2`/`v2.3` не чіпати). 🟡
...
1. **Закрити реліз v2.4** — пуш, `AUDIT.md`, ZIP, тег `v2.4`.
2. **5A (5.1 → 5.2 → 5.3)** — дешеві, помітні покращення прозорості; кожне з оновленням `CHANGELOG`.
